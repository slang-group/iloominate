...
PBS.KIDS.storybook.textArea = function (GLOBAL, PBS, options) {
	...
	var that,
		...
		init = function () {
			...
			if (!initialized) {
				...
				element.style.fontSize = (options.size !== undefined) ? options.size + "pt" : "1em";
				// Set the line height to the options font size to normalize inline font sizes
				element.style.lineHeight = (options.lineHeight !== undefined) ? options.lineHeight + "pt" : "120%";
				...
				for (i = 0; i < textArr.length; i += 1) {
					...
					if (textArr.length === 1) {
						...
						paragraphElements[i] = GLOBAL.document.createElement("textarea");
	...
	element = (textArr.length === 1) ? GLOBAL.document.createElement("textarea") : GLOBAL.document.createElement("div");
	// Inherit the view
	...
	that.height = options && (options.height !== undefined) ? options.height : "220px";
